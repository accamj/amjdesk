...
#define AMJDESK_LIB_PATH "libamjdesk.so"
typedef bool (*AmjDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_amjdesk_core_main() {
   void* libamjdesk = dlopen(AMJDESK_LIB_PATH, RTLD_LAZY);
   if (!libamjdesk) {
      fprintf(stderr,"Failed to load \"libamjdesk.so\"\n");
      char* error;
   ...
   auto core_main = (AmjDeskCoreMain) dlsym(libamjdesk,"amjdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"amjdesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
