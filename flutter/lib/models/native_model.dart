...
class PlatformFFI {
  ...
  AmjdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libamjdesk.so')
        : isLinux
            ? DynamicLibrary.open('libamjdesk.so')
            : isWindows
                ? DynamicLibrary.open('libamjdesk.dll')
                : isMacOS
                    ? DynamicLibrary.open("liblibamjdesk.dylib")
  ...
  void _startListenEvent(AmjdeskImpl amjdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = amjdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
