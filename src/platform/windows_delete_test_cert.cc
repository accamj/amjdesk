// https://github.com/accamj/amjdesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteAmjDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes AmjDesk Test certificates and wrong key stores
//
...
BOOL DeleteAmjDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteAmjDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteAmjDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteAmjDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteAmjDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
